...
**Per-repo fork-setup failure degrades gracefully.** A fork-setup failure for one repository (creation returns non-2xx, e.g. the PAT lacks fork permission or upstream is inaccessible, OR the fork is not reachable within the 60s timeout) does **not** abort startup. autocoder instead: records the failure, **skips that repository for the process lifetime** (no polling task is spawned for it — the same per-repo skip used when a fork URL cannot be derived), and **emits a ChatOps alert** naming the repository and the remedy. Every other repository AND ChatOps keep running. Even if *every* configured repository fails fork setup, the daemon stays up (it never exits non-zero for a per-repo fork-setup failure) so an operator can remediate. **Recovery:** fix the fork (ensure it exists under `fork_owner` and is reachable), then **restart or `reload`** the daemon — the skipped repository's polling task is spawned on the next start/reload once its fork is reachable. (So the alert is deliverable, the ChatOps backend is initialized before fork setup runs.)
