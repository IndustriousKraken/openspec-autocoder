...
Human `@<bot> revise` requests are **uncapped** — they always process. The per-PR cap (default 5; configurable up to 20 via `executor.max_auto_revisions_per_pr`, legacy alias `executor.max_revisions_per_pr`) bounds only **automatic** reviewer-initiated revisions (when `reviewer.auto_revise: true`). Full spec in [OPERATIONS.md → Revising an open PR via comment](OPERATIONS.md#revising-an-open-pr-via-comment).
