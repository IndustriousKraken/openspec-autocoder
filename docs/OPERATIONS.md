...
**Durability.** A queued audit is consumed from `pending_audit_runs` only once it has actually run. A pass that skips (busy marker), returns early (workspace-init failure), or is bounded out (`max_audits_per_iteration: 0`) leaves the queued entry in place for a later iteration — an acknowledged request is never silently dropped. The queue also **survives a daemon restart**: each enqueue and each post-run prune is mirrored (atomic tempfile + rename) to `<state_dir>/pending-audit-runs/<workspace-basename>.json`, and a repo's queue is loaded from that file when its polling task is spawned, so a restart between the `✓ Queued` acknowledgement and the audit's run preserves the request. Persistence is best-effort — a read/write failure is logged and never aborts a run, with the in-memory queue staying authoritative for the live process — and a persisted entry whose repo is no longer configured is reconciled away at startup.

**Completion notification and fail-closed outcomes.** When an operator-triggered audit reaches a terminal outcome, the daemon posts a result back to the originating chatops channel: proposals written (listed), a clean `0 proposals (no findings)` run **with a summary of what the audit examined**, or — per the `gatekeepers-fail-closed` standard — `did NOT complete` when the audit could not reach a verdict (a session error, an uncaptured exit status, or a degenerate empty session that produced no output). A did-not-complete run is surfaced as failed-to-run, **never as "no findings"**: its `last_run_at` cadence state is NOT advanced, so the audit retries on its next cycle — distinct from a genuine clean run. Cadence-driven (non-operator) runs apply the same fail-closed posture, posting a failure alert rather than the operator-thread completion.
...
3. **Pre-flight change-internal contradiction check (a19; opt-in).** Where `a17` catches structural defects, the contradiction check catches semantic ones: a change whose requirements are individually well-formed AND archivable but contradict each other (e.g. ADDED A "all secrets in env vars" + ADDED B "API key in `config.yaml`"). The check runs an agentic CLI session that is handed the change's spec-delta file paths, reads them on demand in a read-only sandbox, AND returns contradictions via the `submit_contradictions` MCP tool (cost is model-dependent). Non-empty findings write the marker with `revision_suggestion` populated from the contradictions narrative — `unarchivable_deltas` AND `unimplementable_tasks` are left empty for this case because the issue is semantic, not mechanical. The check is **disabled by default**; operators trading the small per-change LLM cost for the catch enable it via `executor.change_internal_contradiction_check: enabled` AND `executor.change_internal_contradiction_check_llm`. The check **fails CLOSED** (gatekeepers-fail-closed): a gate that cannot run (CLI unavailable / unregistered strategy, session error, no submission, uncorrected schema rejection) does NOT proceed as "no contradictions" — it logs a WARN AND **holds the change** with the marker's `gate_error` populated + a distinct "gate FAILED TO RUN — change held" alert (distinct from a findings-based revision). The change was NOT evaluated; fix the gate AND `@<bot> clear-revision <repo> <change>` to retry. See [Pre-flight checks](#pre-flight-checks) for the full layered design.
...
| **3. Change-internal contradiction check — the `[in]` gate (a19/a59)** | Agentic semantic check: detects requirements within the same change that cannot all hold simultaneously (e.g. ADDED A "all secrets in env vars" + ADDED B "API key in `config.yaml`"). Runs as a wrapped CLI session that is given the change's spec-delta file **paths**, reads them on demand in a read-only sandbox, AND returns findings via the `submit_contradictions` MCP tool over the control socket — NOT a one-shot HTTP call. Cost is model-dependent. | Agentic CLI session | Opt-in: `executor.change_internal_contradiction_check: enabled` + `executor.change_internal_contradiction_check_llm:` block | Findings → `.needs-spec-revision.json` with `revision_suggestion` from the contradictions narrative. **Fail-CLOSED (held):** a CLI-unavailable / no-submission / malformed result logs a WARN AND **holds the change** — writes the marker with a structured `gate_error` (gate + cause) + posts a distinct "gate FAILED TO RUN — change held" alert. The change was NOT evaluated (NOT waved through); fix the gate AND `clear-revision` to retry. |
| **4. Change-vs-canonical contradiction check — the `[canon]` gate (a62)** | Agentic semantic check, same transport as the `[in]` gate, that catches a change contradicting an ALREADY-canonical requirement (a delta that re-specifies a behavior locked elsewhere in `openspec/specs/`). Reads the change's spec-deltas AND `openspec/specs/*/spec.md` (using `query_canonical_specs` when `canonical_rag` is enabled), returning via the `submit_canon_contradictions` MCP tool. | Agentic CLI session | Opt-in: `executor.change_canonical_contradiction_check: enabled` + `executor.change_canonical_contradiction_check_llm:` block | Same as the `[in]` gate: findings → `.needs-spec-revision.json` with `revision_suggestion`; a gate that cannot run → fail-CLOSED held marker (`gate_error`) + "FAILED TO RUN" alert. |
...
**Why fail-closed (held)?** A gatekeeper that silently passes when it could not run is not a control (the gatekeepers-fail-closed standard). A blocking gate that cannot evaluate a change HOLDS it — visible, operator-cleared — rather than waving it through as "no findings." The hold is distinct from a real finding: the marker's `gate_error` names the gate AND the cause, AND the chatops alert says "FAILED TO RUN — change held," so an operator can tell "could not run" from "found a problem." (A bounded transient retry before holding is a permitted refinement.)

**The `[out]` gate — code-implements-spec verification (a63).** A third verifier gate, but it runs AFTER the executor (so it is NOT a pre-flight) AND is **advisory only**. Enabled via `executor.code_implements_spec_check: enabled` + `executor.code_implements_spec_check_llm:`, it judges — requirement by requirement, scenario by scenario — whether the implementation satisfies the change's spec delta (same agentic transport: a read-only CLI session returning via the `submit_verdict` MCP tool), then renders a `## Spec Verification` section into the PR body (parallel to the reviewer's `## Code Review`) AND posts a chatops note only when gaps are found. It NEVER opens a revision AND NEVER blocks PR creation — the operator reads the verification and decides. On its OWN failure (CLI unavailable, no submission, …) the `[out]` gate fails CLOSED to a VISIBLE state: it renders an explicit `## Spec Verification: FAILED TO RUN` section naming the cause (the change is NOT verified — NOT a pass) rather than silently omitting the section; it still never blocks. Together the three gates (`[in]`, `[canon]`, `[out]`) are the **verifier framework**: `[in]`/`[canon]` are blocking pre-flights that hold the change on failure; `[out]` is post-executor advice that reports FAILED TO RUN on failure.

**Transport (a59/a62/a63).** All three verifier gates run **agentically** — a wrapped CLI session that reads the relevant files on demand in a read-only sandbox AND relays its result via an MCP `submit_*` tool over the daemon control socket — NOT a one-shot HTTP call. Each gate's `*_llm` block's `provider` selects the CLI (`anthropic`→`claude`, `openai_compatible`/`ollama`→`opencode`, `google`→`antigravity`), which must be installed; `api_key_env`/`api_base_url` configure the provider that CLI uses, AND `model` may name a top-level `models:` nickname. See [docs/CONFIG.md](CONFIG.md) for the field surface.
