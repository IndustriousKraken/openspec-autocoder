...
| `change_internal_contradiction_check_llm` | required when enabled | _absent_ | LLM block for the `[in]` gate. Fields parallel the `reviewer:` LLM surface — `provider` (`anthropic` \| `openai_compatible` \| `ollama`), `model`, `api_key_env` / `api_key`, `api_base_url`. `provider` may be **omitted** to reference a [`models:` nickname](#models-optional) (its `model` is then the nickname). Held as its own block so operators can pick a cheaper model than the reviewer (the prompt is small; on a can't-run failure the `[in]` gate fails CLOSED — it holds the change rather than waving work through, per the [gatekeepers-fail-closed](OPERATIONS.md#pre-flight-checks) standard, with bounded transient retry first via `verifier_gate_retries`). **Transport:** all three verifier gates run **agentically** — a wrapped CLI session that reads files on demand in a read-only sandbox and relays its result via an MCP `submit_*` tool over the daemon control socket, **not** a one-shot HTTP call. `provider` selects the CLI (`anthropic`→`claude`, `openai_compatible`/`ollama`→`opencode`, `google`→`antigravity`), which must be installed; `api_base_url` configures the provider endpoint. **`api_key` is OPTIONAL** for the verifier gates (CLI/agentic roles self-authenticate from the CLI's own login/store); a key is required only for an in-process HTTP consumer (the `oneshot` reviewer / RAG). If you *do* supply one, it is passed to the CLI and is then readable by the sandboxed model — an opt-in exposure ([SECURITY.md → Supplied LLM keys](SECURITY.md#supplied-llm-keys-for-cli-roles-opt-in-exposure)). See [Pre-flight checks](OPERATIONS.md#pre-flight-checks). |
| `change_canonical_contradiction_check` | no | `disabled` | Opt-in gate for the change-vs-canonical contradiction pre-flight — the `[canon]` gate (a62). `disabled` (default) skips it. `enabled` runs alongside the `[in]` gate, BEFORE the executor; it reads the change's spec-deltas AND `openspec/specs/*/spec.md` (using `query_canonical_specs` when `canonical_rag` is on) and catches a change that contradicts an already-canonical requirement. Non-empty findings write `.needs-spec-revision.json` and halt the queue walk. Enabling without `change_canonical_contradiction_check_llm` is a fail-fast startup error. See [Pre-flight checks](OPERATIONS.md#pre-flight-checks). |
| `change_canonical_contradiction_check_prompt_path` | no | _embedded_ | Override for the built-in `[canon]`-check prompt template (embedded `prompts/change-vs-canonical-check.md`). An empty override file is rejected at use time. |
| `change_canonical_contradiction_check_llm` | required when enabled | _absent_ | LLM block for the `[canon]` gate. Same surface AND agentic transport as `change_internal_contradiction_check_llm` (see its row); pick a model independently. |
| `code_implements_spec_check` | no | `disabled` | Opt-in gate for code-implements-spec verification — the `[out]` gate (a63). `disabled` (default) skips it AND spawns no post-executor session. `enabled` runs AFTER the executor in a read-only sandbox, judging requirement-by-requirement whether the implementation satisfies the change's spec delta, and renders an advisory `## Spec Verification` PR-body section (parallel to the reviewer's `## Code Review`) plus a chatops note only when gaps are found. **Advisory: it NEVER opens a revision AND NEVER blocks PR creation.** Enabling without `code_implements_spec_check_llm` is a fail-fast startup error. See [Pre-flight checks](OPERATIONS.md#pre-flight-checks). |
| `code_implements_spec_check_prompt_path` | no | _embedded_ | Override for the built-in `[out]`-check prompt template (embedded `prompts/code-implements-spec-check.md`). An empty override file is rejected at use time. |
| `code_implements_spec_check_llm` | required when enabled | _absent_ | LLM block for the `[out]` gate. Same surface AND agentic transport as `change_internal_contradiction_check_llm` (see its row); pick a model independently. |
| `verifier_gate_retries` | no | `2` | Bounded retry of an agentic verifier-gate session ([in]/[canon]/[out]) that ends with NO submission — the flaky case where a weak local model reads the spec then never calls its `submit_*` MCP tool. The same model+change submits non-deterministically, so a bounded retry catches the "sometimes works" case before the gate fails closed. Counts ADDITIONAL attempts: `2` (default) is up to 3 total attempts; `0` disables retry (one attempt — the historical behavior). Only the no-submission case retries — a successful submission, a timeout, AND an unregistered-strategy / CLI-unavailable error are NOT retried. This is the [gatekeepers-fail-closed](OPERATIONS.md#pre-flight-checks) standard's bounded-retry tolerance: after the bound is exhausted the gate still fails closed ([in]/[canon] hold the change; [out] renders an advisory `## Spec Verification: FAILED TO RUN`), never fail-open. |
