...
|---|---|---|
| `defaults` | `map<audit-slug, Cadence>` | Global default cadence per audit type. Audit slugs must match a registered type (currently `architecture_brightline`, `architecture_consultative`, `drift_audit`, `missing_tests_audit`, `security_bug_audit`, `spec_sync_audit`); typos fail at config load with a list of known slugs. |
...
You type the short name; the bot resolves it. The match is case-insensitive substring search against the full configured `repositories[].url`. `coterie` matches `git@github.com:IndustriousKraken/coterie.git`; `COTERIE` does too. If two repos with the same trailing name exist under different owners, the bot replies with the candidate list and asks for a more specific substring. If nothing matches, the bot replies with the full list of configured URLs so you can copy one back.
...
```
⚠️ This will delete /tmp/workspaces/github_com_IndustriousKraken_coterie (forces a re-clone on the next iteration). Reply 'confirm' within 60 seconds.
```
...
| `spec_sync_audit` | Walks every archived change's `specs/<capability>/spec.md` deltas and merges any ADDED/MODIFIED/REMOVED/RENAMED requirements that haven't yet landed in the canonical `openspec/specs/<capability>/spec.md` files. Idempotent — most runs find nothing. Useful for: repos that pre-date the OpenSpec `archive`/`sync` split, repos onboarded with pre-existing drift, repos where operators occasionally run `openspec archive` by hand without the sync skill installed. When drift IS present, the audit's first run produces one commit titled `audit: spec-sync — merge deltas from N archived change(s)` that backfills the entire gap; subsequent runs are noops until new drift appears. | No | `disabled` (opt-in via `audits.defaults` or per-repo) | `CanonicalSpecMerge` (writes constrained to `openspec/specs/**`; post-hoc diff check reverts anything outside that prefix) |

**Caveat on `spec_sync_audit`:** This audit exists because OpenSpec 0.18+ split sync into a separate skill that isn't installed in the core profile; the upstream is expected to re-bundle in a future release. Until then (and as a defensive backstop afterward), this audit catches the drift.
...
- **`CanonicalSpecMerge`** — sandbox allows `Write`/`Edit`; after `run()` returns every modified or new path must begin with `openspec/specs/`. A diff outside that prefix triggers the same failure handling. Used by `spec_sync_audit` (and any future audit that mechanically merges committed history into canonical specs).
