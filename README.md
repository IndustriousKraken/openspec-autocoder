...
**Updating with `update.sh` (recommended).** To update on demand, run `update.sh` directly — it resolves the latest non-prerelease release, downloads + checksum-verifies the binary, runs `autocoder check-config` as a preflight, atomically swaps the running binary aside to `/usr/local/bin/autocoder.previous`, restarts the systemd unit, and **rolls back automatically** if the daemon does not reach `active` within 30 seconds. That auto-rollback is why running `update.sh` is the safer in-place upgrade path over re-running `install.sh`. Use `--dry-run` to preview and `--version vX.Y.Z` to pin a tag. See [docs/DEPLOYMENT.md → Updating on demand](docs/DEPLOYMENT.md#updating-on-demand) for staging it, and the cron section there for unattended scheduling. **Not recommended** for enterprise change-control environments where Ansible / container registries / k8s pipelines already own update orchestration.
