...
autocoder's archive step shells out to `openspec archive`, which performs both the file move AND the merge of change deltas into canonical capability specs — but the merge step is only available when `Sync specs` is enabled in the openspec profile (in openspec 0.18+ the sync workflow is opt-in, see [Fission-AI/OpenSpec#913](https://github.com/Fission-AI/OpenSpec/issues/913)). Without it, `openspec archive` moves the change directory but won't update canonical specs; autocoder iterations succeed but drift accumulates in `openspec/specs/`.

To reconcile drift after the fact (e.g. for repos with pre-existing drift, or after onboarding a repo from a host that didn't have `sync` enabled), see the companion `rebuild-canonical-specs-from-archive` change.
