...
- [ ] 1.1 Locate the status reply composer (likely in `autocoder/src/chatops/status.rs` or similar). The current code reads the busy marker AND produces either `idle` or `working on <change> (started <age> ago)`.
- [ ] 1.2 Refactor the `currently:` line logic to a single function that branches on marker contents in this order:
...
- [ ] 1.3 The age formatting matches the existing convention (`3m ago`, `2h17m ago` — hours+minutes for older).

## 2. Audit-type lookup

- [ ] 2.1 When the marker has `stage=executor` AND `change` is empty, the status code attempts to identify which audit is running by scanning the audit logs directory:
  ```
...
- [ ] 2.2 The lookup uses the daemon's resolved logs-dir path (per `a09`). No hard-coded `/tmp/autocoder/logs/...` literals.
- [ ] 2.3 Tests:
...
- [ ] 3.1 The status code reuses `a08`'s classification helper. When the helper says "this marker would trigger recovery on the next iteration" (dead pid OR age ≥ threshold), the status emits the stale-marker line.
- [ ] 3.2 Format:
...
- [ ] 4.1 Identify which recovery operations stamp distinguishable markers today: rebuild-specs, fork recreation. Each likely sets a unique `stage` value or extends the marker schema. Read the actual marker contents and surface them in the status reply.
- [ ] 4.2 If no distinguishable marker shape exists for a recovery operation, the spec doesn't add one — the operation falls through to the generic `<stage> in progress` line.

## 5. Docs

- [ ] 5.1 In `docs/CHATOPS.md`'s operator-recovery-commands section, extend the `status` verb's reply-shape examples to include each new variant:
  ```
  currently: idle
  ...
  ```
- [ ] 5.2 Add a paragraph explaining the diagnostic value: operators wondering why a pending change isn't being picked up can read the `currently:` line AND distinguish "audit in flight, just wait" from "stale marker, need a08's recovery to fire (or manual `rm`)".

## 6. Spec deltas

- [ ] 6.1 `openspec/changes/a11-status-surfaces-busy-marker-contents/specs/chatops-manager/spec.md` MODIFIES the existing `Status reply always shows live workspace snapshot` requirement to add scenarios for each new variant.
- [ ] 6.2 `openspec/changes/a11-status-surfaces-busy-marker-contents/specs/project-documentation/spec.md` ADDs one requirement covering the CHATOPS.md documentation update.

## 7. Verification

- [ ] 7.1 `cargo test` passes (new + existing).
- [ ] 7.2 `openspec validate a11-status-surfaces-busy-marker-contents --strict` passes.
