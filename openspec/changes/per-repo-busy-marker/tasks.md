...
- [ ] 1.1 Create `autocoder/src/busy_marker.rs`. Public types:
    ```rust
    ...
    ```
- [ ] 1.2 Implement `pub fn marker_path(workspace_basename: &str) -> PathBuf` returning `/tmp/autocoder/busy/<basename>.json` via `std::env::temp_dir().join("autocoder").join("busy").join(format!("{basename}.json"))`.
- [ ] 1.3 Implement `pub fn try_acquire(workspace: &Path, repo_url: &str, stuck_threshold_secs: u64) -> Result<AcquireOutcome>`:
...
- [ ] 1.4 Implement `BusyGuard::set_stage(&self, stage: Stage)` that rewrites the JSON via atomic write-temp-then-rename (`{path}.tmp` → `rename(.tmp, path)`).
- [ ] 1.5 Implement `Drop for BusyGuard` that calls `std::fs::remove_file(&self.path)` with a `tracing::warn!` if it errors (best-effort).
- [ ] 1.6 **Verify:** unit tests `busy_marker::tests::*`:
...
- [ ] 2.1 In `claude_cli::run_subprocess`, after constructing the `Command` and before `.spawn()`, add `command.process_group(0)` (stable since Rust 1.64) so the child becomes a new process-group leader. This lets the busy-marker code `killpg` the whole subprocess tree.
- [ ] 2.2 **Verify:** existing executor tests continue to pass — `process_group(0)` doesn't change exit behavior, only group membership. No new test needed for the group setup itself (POSIX semantics are stable); the stuck-state-kill test in §1.6 covers the consuming behavior.

## 3. Run-log path unification

- [ ] 3.1 In `claude_cli.rs`, modify `run_log_path` to return `<temp>/autocoder/logs/<basename>/<change>.log` instead of `<temp>/autocoder-logs/<basename>/<change>.log`.
- [ ] 3.2 Update the test `run_log_path_is_under_workspace_basename_and_change_name` to assert the new path layout.
- [ ] 3.3 No backwards-compat shim. Old logs at `/tmp/autocoder-logs/` become unreferenced; they'll age out via `/tmp` cleanup. README documents the new path.

## 4. Polling-loop integration

- [ ] 4.1 At the top of `polling_loop::execute_one_pass`, after workspace path resolution but before any other work, call `busy_marker::try_acquire(workspace, &repo.url, repo.executor_timeout_secs + 600)`. Return early on `SkipFreshInProgress` (INFO log) and `SkipAmbiguous` (ERROR log + chatops alert). The threshold uses `cfg.executor.timeout_secs` reached via the executor config — the polling-loop signature may need a tweak to thread this value through.
- [ ] 4.2 On `Acquired(guard)`, bind the guard to a local variable for the duration of the function. Drop happens automatically at every return path.
- [ ] 4.3 At each stage transition (before the executor, after walk_queue, before review, before push, before PR), call `guard.set_stage(...)`. This requires holding a `&BusyGuard` accessible to the right code paths — pass through `execute_one_pass`'s helpers as needed.
- [ ] 4.4 For the chatops alerts on stuck/ambiguous states: reuse the `handle_predictable_failure` helper introduced by `chatops-progress-notifications` once that change re-lands, OR if it has not re-landed yet, write a small inline `post_notification` call gated on `chatops_ctx.is_some()`. Mark the chatops integration as a soft dependency in the proposal.
- [ ] 4.5 **Verify:** integration tests in `polling_loop::tests`:
...
- [ ] 5.1 README "Operating Notes": new subsection "Busy marker" describing the file's purpose, location, contents, lifecycle, and what an operator should do if they see a stuck marker. No kitschy framing — declarative.
- [ ] 5.2 README "Operating Notes": update the existing workspace-recovery subsection to mention that the run-log path is now `/tmp/autocoder/logs/...` not `/tmp/autocoder-logs/...`.
- [ ] 5.3 README "Operating Notes": brief note that operators can `cat /tmp/autocoder/busy/<basename>.json` to see what stage the daemon is in for any active repo.

## 6. Verification

- [ ] 6.1 `cargo test` passes.
- [ ] 6.2 `openspec validate per-repo-busy-marker --strict` passes.
- [ ] 6.3 `cargo build --release` produces a binary that, on a clean `/tmp`, runs a polling pass that creates `/tmp/autocoder/busy/<basename>.json`, holds it through the pass, and removes it on completion. Verifiable by `watch ls /tmp/autocoder/busy/` during a manual run.
