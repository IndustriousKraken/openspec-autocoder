...
- [ ] 1.1 New file `prompts/drift-audit.md`. Contents:
...
- [ ] 1.2 Embed at compile time via `include_str!("../../prompts/drift-audit.md")` in `audits/drift.rs`.

## 2. Audit implementation

- [ ] 2.1 New module `autocoder/src/audits/drift.rs`. Define `pub struct DriftAudit { settings: AuditSettings, executor_command: String, executor_timeout_secs: u64 }`.
- [ ] 2.2 `impl Audit for DriftAudit` with `audit_type() = "drift_audit"`, `requires_head_change() = true`, `write_policy() = WritePolicy::None`.
- [ ] 2.3 `run(&self, ctx)` algorithm:
...
- [ ] 2.4 Reuse `ClaudeCliExecutor::write_sandbox_settings`-style logic if cleanly extractable; otherwise duplicate the small block locally. Prefer a shared helper in `audits/mod.rs` that returns the path + a `TempFileGuard`, so the drift audit and future LLM audits don't each reimplement it.

## 3. Registration

- [ ] 3.1 In `cli/run.rs::build_audit_registry`, append `Arc::new(DriftAudit::new(&audit_settings, &cfg.executor))`.
- [ ] 3.2 Tests in `audits::drift::tests`:
...
- [ ] 4.1 README "Periodic audits" — add `drift_audit` to the registered-audits list. Document: triggers on HEAD change at the configured cadence, is purely advisory, never modifies code or specs.
- [ ] 4.2 README "Config reference" — under `audits.drift_audit`, document `prompt_path` (override default), `notify_on_clean` (post a "no findings" message), and any drift-specific knobs.

## 5. Verification

- [ ] 5.1 `cargo test` passes.
- [ ] 5.2 `openspec validate drift-audit --strict` passes.
