...
//! agent's job. The audit's job is to (a) hand the agent a sandboxed
//! workspace and the right prompt, (b) detect which change directories
//! the agent created (by diffing a pre-run snapshot of
//! `openspec/changes/` against the post-run state), (c) validate each
//! new directory via `openspec validate <name> --strict`, (d) commit
//! the validated ones to the agent branch, and (e) return
//! `AuditOutcome::SpecsWritten(validated_names)` so the same iteration's
//! `walk_queue` picks them up immediately.
//!
...
use super::{
    Audit, AuditContext, AuditOutcome, WritePolicy, write_sandbox_settings,
};
use crate::config::{AuditSettings, ExecutorConfig, ResolvedSandbox};

/// Tools the missing-tests agent may call. Includes `Write` and `Edit`
/// because the agent's whole job is to create OpenSpec change files.
const ALLOWED_TOOLS: &[&str] = &["Read", "Glob", "Grep", "Bash", "Write", "Edit"];
...
impl Audit for MissingTestsAudit {
    ...
    async fn run(&self, ctx: &mut AuditContext<'_>) -> Result<AuditOutcome> {
        ...
        // Snapshot existing change directories so we can identify which
        // ones the agent newly created.
        let before: HashSet<String> = snapshot_change_dirs(ctx.workspace);

        // Force the allowed_tools list per the spec; defer everything
        // else (deny patterns) to the executor's resolved sandbox.
        ...
        let (settings_path, _settings_guard) =
            write_sandbox_settings(&sandbox, self.settings_dir.as_deref())
                .context("generating missing-tests-audit sandbox settings file")?;

        let _ = ctx.log_writer.write_section(
            "missing_tests_preamble",
            &format!(
                "executor_command: {}\ntimeout_secs: {}\nprompt_source: {}\nmax_proposals_per_run: {}\nsettings_file: {}\nallowed_tools: {}\npre_run_change_dirs: {}",
        ...
        let _ = ctx
            .log_writer
            .write_section("missing_tests_prompt", &prompt);
        ...
        .await
        .context("spawning missing-tests-audit CLI subprocess")?;

        let _ = ctx.log_writer.write_section(
            "missing_tests_stdout",
        ...
        if outcome.timed_out {
            let _ = ctx.log_writer.write_section(
                "missing_tests_outcome",
                "kind: Err\nreason: timeout",
            );
            return Err(anyhow!(
                "missing_tests_audit: CLI exceeded the {}s timeout",
                self.executor_timeout_secs
            ));
        }
        ...
        if let Some(status) = outcome.exit_status {
            if !status.success() {
                ...
                return Err(anyhow!(
                    "missing_tests_audit: CLI exited {status}"
                ));
            }
        }

        // Diff the post-run state against the snapshot to find new
        // change directories.
        let after = snapshot_change_dirs(ctx.workspace);
        let mut new_dirs: Vec<String> = after.difference(&before).cloned().collect();
        new_dirs.sort();

        // Cap is enforced by the prompt itself, but defend in depth:
        // if the agent produced more than `max_proposals_per_run`, drop
        // the surplus (deterministic by sorted name) so the audit never
        // commits more than it advertised.
        let cap = self.max_proposals_per_run as usize;
        if new_dirs.len() > cap {
            ...
            for d in &dropped {
                ...
                if let Err(e) = std::fs::remove_dir_all(&path) {
                    tracing::warn!(
                        path = %path.display(),
                        "failed to remove over-cap missing-tests change dir: {e}"
                    );
                }
            }
            let _ = ctx.log_writer.write_section(
                "missing_tests_dropped_over_cap",
                &format!(
                    "cap: {}\ndropped:\n{}",
                    self.max_proposals_per_run,
                    dropped.join("\n")
        ...
        // Validate each newly-created change. Invalid ones are removed
        // so the framework's post-hoc check sees a clean openspec/changes/
        // subtree and the committed work is strictly the validated set.
        let mut validated: Vec<String> = Vec::new();
        for name in &new_dirs {
            match validate_change(&self.openspec_command, ctx.workspace, name).await {
                ...
                Err(e) => {
                    ...
                    if let Err(rm_err) = std::fs::remove_dir_all(&path) {
                        tracing::warn!(
                            path = %path.display(),
                            "failed to remove invalid missing-tests change dir: {rm_err}"
                        );
                    }
                    let _ = ctx.log_writer.write_section(
                        &format!("missing_tests_validation_failure_{name}"),
                        &format!("change: {name}\nerror: {e:#}"),
                    );
                    tracing::warn!(
                        change = %name,
                        "missing_tests_audit: rejecting agent-produced change that failed `openspec validate --strict`: {e:#}"
                    );
                }
            }
        }
        ...
        // Commit the validated changes to the agent branch. Stage only
        // under `openspec/changes/` so a stray edit elsewhere (which
        // the framework's post-hoc check will catch and revert) does
        // not silently piggyback into the audit's commit.
        git_add_openspec_changes(ctx.workspace)
            .context("staging missing-tests-audit's openspec/changes/ for commit")?;
        let commit_msg = format!(
            "audit: missing-tests proposals ({} change(s))",
            validated.len()
        );
        crate::git::commit(ctx.workspace, &commit_msg)
            .with_context(|| {
                format!(
                    "committing missing-tests-audit's {} change(s)",
                    validated.len()
                )
            })?;
...
/// directory is absent (fresh repo with no changes yet).
pub(crate) fn snapshot_change_dirs(workspace: &Path) -> HashSet<String> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(name) = entry.file_name().to_str() {
            // Skip the `archive/` directory; archived changes are not
            // in-flight and should not count toward the "new" diff.
            if name == "archive" {
                continue;
            }
...
/// Run `<openspec_command> validate <name> --strict` in `workspace`.
/// Returns `Ok(())` on a zero exit, `Err` on anything else with the
/// stderr tail attached so the audit log captures why validation failed.
async fn validate_change(
    ...
    let output = Command::new(openspec_command)
        ...
        .with_context(|| {
            format!(
                "spawning `{openspec_command} validate {change_name} --strict`"
            )
        })?;
...
/// `git add openspec/changes/` — narrowly stage just the audit's
/// allowed write surface, NOT the whole tree.
fn git_add_openspec_changes(workspace: &Path) -> Result<()> {
    ...
    if !status.success() {
        return Err(anyhow!(
            "`git add openspec/changes/` exited {status}"
        ));
    }
    Ok(())
}
...
/// Spawn the wrapped CLI with the prompt on stdin. Mirrors the drift
/// audit's runner — same flags (`--settings`, `--allowedTools`,
/// `--permission-mode acceptEdits`) and the same timeout discipline.
async fn run_subprocess(
    ...
    let mut child = Command::new(command)
        ...
        .process_group(0)
        .spawn()
        .with_context(|| format!("spawning missing-tests-audit command `{command}`"))?;
    ...
    match exit_status {
        ...
        Some(Err(e)) => Err(e).context("waiting on missing-tests-audit child process"),
        Some(Ok(status)) => {
...
mod tests {
    ...
    async fn validation_failure_rejects_change_and_logs_warning() {
        ...
        assert!(
            log.contains("missing_tests_validation_failure_tests-bad-shape"),
